...
const ThreeDModel = ({
  ...
  // State for interactive features - all interaction disabled
  ...
  useEffect(() => {
    ...
    camera.lookAt(0, modelType === "laptop" ? -0.3 : 0, 0);
    ...
    } else if (modelType === "abstract") {
      model.rotation.x = -Math.PI / 20;
      model.rotation.y = Math.PI / 4;
    } else {
      // Laptop model
      model.rotation.x = -Math.PI / 12;
      model.rotation.y = Math.PI / 5; // Rotate slightly to show keyboard and screen
    }
    model.rotation.z = 0;
    scene.add(model);
    ...
    const animate = (time: number) => {
      ...
      // Apply constant gentle rotation for auto-animation only
      if (!scrollTrigger) {
      ...
      if (particlesRef.current && modelType === "abstract") {
        ...
        for (let i = 0; i < positions.length; i += 3) {
          // Wave effect
          positions[i + 1] +=
            Math.sin(time * 0.001 + positions[i] * 0.1) * 0.01;
        }

        particlesRef.current.geometry.attributes.position.needsUpdate = true;
      }

      // Removed interactive lighting effects

      // Limit framerate on mobile for performance
      const elapsed = time - lastTime;
      if (isMobile && elapsed < 33) {
        // ~30fps on mobile
    ...
    // Only keep scroll-based animations if scrollTrigger is enabled
    const handleScroll = () => {
      if (!modelRef.current || !sceneRef.current || !scrollTrigger) return;
      ...
      if (isVisible && modelRef.current) {
        ...
        if (scrollTrigger) {
          gsap.to(modelRef.current.rotation, {
            y: containerVisibleRatio * Math.PI * 2,
            x: -Math.PI / 12 + containerVisibleRatio * 0.3,
            duration: 0.7,
            ease: "power2.out",
          });
    ...
    function createLaptopModel(): THREE.Group {
      ...
      const screenDisplayGeometry = new THREE.PlaneGeometry(
        screenWidth - 0.2,
        screenHeight - 0.2,
        1,
        1
          ...
          for (let y = 1; y <= 30; y++) {
            ...
            for (let i = 0; i < elementCount; i++) {
              ...
              if (codeElements.includes(code)) {
                ...
                const colorKeys = Object.keys(syntaxColors) as Array<
                  keyof typeof syntaxColors
                >;
                ctx.fillStyle =
                  syntaxColors[
      ...
      if (!isMobile) {
        ...
        const touchpadGeometry = new THREE.BoxGeometry(
          touchpadWidth,
          0.02,
          touchpadHeight
        );
    ...
    function createWorkspaceModel(): THREE.Group {
      ...
      // Add a monitor
      const monitorStandGeometry = new THREE.CylinderGeometry(
        0.1,
        0.15,
        0.5,
        16
    ...
    function createAbstractModel(): THREE.Group {
      ...
      for (let i = 0; i < numShapes; i++) {
        ...
        const shapeFunction =
          shapeFunctions[Math.floor(Math.random() * shapeFunctions.length)];
        ...
        const animateShapes = (time: number) => {
          abstractGroup.children.forEach((child) => {
            ...
            mesh.position.y =
              data.initialY +
              Math.sin(time * 0.001 * data.floatSpeed + data.phase) *
                data.floatDistance;
          });
    ...
    function createCodeSceneModel(): THREE.Group {
      ...
      for (let i = 0; i < ringCount; i++) {
        ...
        const orbitGeometry = new THREE.TorusGeometry(orbitRadius, 0.03, 8, 64);
    ...
    function addParticleSystem(
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        if (isCodeParticles) {
          ...
          particles[i3] =
            gridX * (particleSpread / 10) +
            (Math.random() - 0.5) * (particleSpread / 5);
          particles[i3 + 1] =
            gridY * (particleSpread / 10) +
            (Math.random() - 0.5) * (particleSpread / 5);
          particles[i3 + 2] =
            gridZ * (particleSpread / 10) +
            (Math.random() - 0.5) * (particleSpread / 5);

          // Digital appearance
          particleSizes[i] =
            Math.random() > 0.8 ? particleSize * 2 : particleSize;
    ...
    return () => {
      ...
      if (rendererRef.current) {
        ...
        if (rendererRef.current.domElement) {
          ...
          if (
            parent &&
            parent.contains &&
            parent.contains(rendererRef.current.domElement)
          ) {
            try {
      ...
      // Remove event listeners for resize and scroll
  ...
  return (
    <div
      ...
      style={{ touchAction: "none", pointerEvents: "none" }} // Prevent all interactions
    >
